...
\end{flushleft}
\subsection{Four major challenges}
\begin{flushleft}
    \textbf{Cryptocurrencies today still face four major challenges}: monopolization, centralization of computing power, high energy consumption, and shortcomings in the existing PoW mechanism. DePINC has emerged to address these issues. It excels over existing cryptocurrencies in the following aspects:
\end{flushleft}
\begin{flushleft}
    \textbf{Monopoly Issue}: By utilizing hard drives as consensus participants, DePINC reduces the barriers to entry for mining, preventing resources from being concentrated in the hands of a few large miners.
\end{flushleft}
\begin{flushleft}
    \textbf{Centralization of Computing Power}: The PoST mechanism decreases reliance on computational power, enabling more individuals to participate in the network and thereby enhancing decentralization.
\end{flushleft}
\begin{flushleft}
    \textbf{High Energy Consumption}: Compared to PoW, DePINC's CPoST consensus mechanism offers lower energy consumption, promoting environmental sustainability.
\end{flushleft}
\begin{flushleft}
    \textbf{Shortcomings of Existing PoW Mechanism}: DePINC addresses these by integrating the PoST consensus mechanism, comprehensively enhancing system security and stability both technically and within the community.
\end{flushleft}
...
\begin{flushleft}
    The DePINC wallet is derived from Bitcoin and the consensus from Chia's PoST.
\end{flushleft}
\begin{flushleft}
...
\end{flushleft}
\begin{flushleft}
    Chia started in August 2017. Combining the advantages of Bitcoin and Chia, DePINC has currently become the most reliable public chain with PoST consensus algorithm.
\end{flushleft}
\begin{flushleft}
...
\end{flushleft}
\begin{flushleft}
    By adopting the mature PoST mechanism, a stable and reliable consensus mechanism is introduced to build community confidence in the DePINC public chain. Since being compatible with Chia Plot files, miners can get both DePINC and Chia benefits, with only an additional operation.
\end{flushleft}
\begin{flushleft}
...
\end{flushleft}
\begin{flushleft}
    When it comes to PoST, due to the relatively low power consumption by hardware, miners can obtain other coins in the future symbiotic ecosystem of PoST almost free of charge without any risk.
\end{flushleft}
\begin{flushleft}
